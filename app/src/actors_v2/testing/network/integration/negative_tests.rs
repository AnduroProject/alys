...
use crate::actors_v2::network::{
    NetworkActor, NetworkConfig, NetworkMessage, NetworkResponse, NetworkError,
};
...
async fn test_invalid_multiaddr_format() {
    ...
    for invalid_addr in invalid_addrs {
        let result = actor.send(NetworkMessage::StartNetwork {
        ...
        match result {
            ...
            Ok(Err(NetworkError::Internal(_))) => {
                println!("Correctly rejected invalid address with internal error: {}", invalid_addr);
            }
            Ok(Err(e)) => {
    ...
    // Cleanup
    actor.send(NetworkMessage::StopNetwork { graceful: false }).await.ok();
...
async fn test_port_already_in_use() {
    ...
    let start_result1 = actor1.send(NetworkMessage::StartNetwork {
    ...
    let start_result2 = actor2.send(NetworkMessage::StartNetwork {
        listen_addrs: vec!["/ip4/127.0.0.1/tcp/19002".to_string()],
        bootstrap_peers: vec![],
    }).await;
    ...
    // Cleanup
    actor1.send(NetworkMessage::StopNetwork { graceful: false }).await.ok();
    actor2.send(NetworkMessage::StopNetwork { graceful: false }).await.ok();
...
async fn test_invalid_bootstrap_peer() {
    ...
    match status {
        NetworkResponse::Status(s) => {
            println!("Actor status after invalid bootstrap: running={}", s.is_running);
...
async fn test_operations_before_network_started() {
    ...
    let broadcast_result = actor.send(NetworkMessage::BroadcastBlock {
    ...
    let request_result = actor.send(NetworkMessage::RequestBlocks {
        start_height: 0,
        count: 10,
        correlation_id: None,
    }).await;
...
async fn test_invalid_block_request_parameters() {
    ...
    let result = actor.send(NetworkMessage::RequestBlocks {
        start_height: 0,
        count: 0,
        correlation_id: None,
    }).await;
...
async fn test_invalid_auxpow_data() {
    ...
    let result = actor.send(NetworkMessage::BroadcastAuxPow {
...
async fn test_repeated_start_stop() {
    ...
    // Start network
    let start1 = actor.send(NetworkMessage::StartNetwork {
        listen_addrs: vec!["/ip4/127.0.0.1/tcp/19008".to_string()],
        bootstrap_peers: vec![],
    }).await.expect("Failed to send start").expect("Start 1 failed");

    assert!(matches!(start1, NetworkResponse::Started));
    sleep(Duration::from_millis(500)).await;

    // Try to start again (should be idempotent)
    let start2 = actor.send(NetworkMessage::StartNetwork {
        listen_addrs: vec!["/ip4/127.0.0.1/tcp/19008".to_string()],
        bootstrap_peers: vec![],
    }).await.expect("Failed to send start").expect("Start 2 failed");

    assert!(matches!(start2, NetworkResponse::Started));
    println!("Repeated start handled correctly (idempotent)");

    // Stop network
    let stop1 = actor.send(NetworkMessage::StopNetwork { graceful: true })
        .await.expect("Failed to send stop").expect("Stop 1 failed");

    assert!(matches!(stop1, NetworkResponse::Stopped));
    sleep(Duration::from_millis(500)).await;

    // Try to stop again (should be idempotent)
    let stop2 = actor.send(NetworkMessage::StopNetwork { graceful: true })
        .await.expect("Failed to send stop").expect("Stop 2 failed");
...
async fn test_shutdown_modes() {
    ...
    let graceful_result = actor1.send(NetworkMessage::StopNetwork { graceful: true })
        .await.expect("Failed to send stop").expect("Graceful stop failed");
    ...
    let immediate_result = actor2.send(NetworkMessage::StopNetwork { graceful: false })
        .await.expect("Failed to send stop").expect("Immediate stop failed");
...
async fn test_connection_to_unreachable_peer() {
    ...
    match status {
        NetworkResponse::Status(s) => {
            assert!(s.is_running, "Actor should still be running after failed connections");
            println!("Actor operational after connection failures: peers={}", s.connected_peers);
        }
        _ => panic!("Unexpected status response"),
    }
