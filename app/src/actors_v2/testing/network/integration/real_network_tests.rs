...
async fn test_real_tcp_connection_establishment() {
    ...
    match status1 {
        NetworkResponse::Status(status) => {
            ...
            println!(
                "Actor1 status: running={}, peers={}",
                status.is_running, status.connected_peers
            );
        }
...
async fn test_gossipsub_message_delivery() {
    ...
    match metrics1 {
        NetworkResponse::Metrics(m) => {
            println!(
                "Actor1 metrics: msgs_sent={}, gossip_published={}",
                m.messages_sent, m.gossip_messages_published
            );
            assert!(
                m.gossip_messages_published > 0,
                "Should have published gossip messages"
            );
        }
...
async fn test_request_response_protocol() {
    ...
    let peers_result = actor2
        .send(NetworkMessage::GetConnectedPeers)
        .await
    ...
    match request_result {
        Ok(Ok(NetworkResponse::BlocksRequested {
            peer_count,
            request_id,
        })) => {
            println!(
                "Block request sent to {} peers, request_id: {}",
                peer_count, request_id
            );
        }
        Ok(Err(e)) => {
            println!(
                "Block request error (expected if no suitable peers): {:?}",
                e
            );
        }
    ...
    match metrics2 {
        NetworkResponse::Metrics(m) => {
            println!(
                "Actor2 metrics: block_requests_sent={}",
                m.block_requests_sent
            );
...
async fn test_multi_peer_topology() {
    ...
    actor3
        ...
        .await
        .expect("Failed to start actor3")
        .expect("Actor3 start failed");
    ...
    for (name, actor) in [
        ...
        match status {
            NetworkResponse::Status(s) => {
                println!(
                    "{}: running={}, peers={}",
                    name, s.is_running, s.connected_peers
                );
...
async fn test_auxpow_broadcast() {
    ...
    use lighthouse_wrapper::types::{Address, Hash256};
    ...
    match metrics1 {
        NetworkResponse::Metrics(m) => {
            println!(
                "Actor1 AuxPoW metrics: broadcasts={}, bytes={}",
...
async fn test_connection_recovery() {
    ...
    actor2_new
        ...
        .await
        .expect("Failed to restart actor2")
        .expect("Actor2 restart failed");
    ...
    match status1 {
        NetworkResponse::Status(s) => {
            assert!(
                s.is_running,
                "Actor1 should still be running after recovery"
            );
            println!(
                "Actor1 status after recovery: running={}, peers={}",
