...
impl Actor for ValidationWorker {
    ...
    fn started(&mut self, _ctx: &mut Self::Context) {
        tracing::info!("ValidationWorker {} started", self.id);
    }

    fn stopped(&mut self, _ctx: &mut Self::Context) {
        tracing::info!("ValidationWorker {} stopped", self.id);
    }
}
...
impl Handler<ValidateBlockMessage> for ValidationWorker {
    ...
    fn handle(&mut self, msg: ValidateBlockMessage, _ctx: &mut Self::Context) -> Self::Result {
        ...
        async move {
            ...
            tracing::debug!("Worker {} completed validation: {:?}", worker_id, result.is_valid);
        }
        .into_actor(self)
        .boxed_local()
    }
...
impl Handler<ShutdownWorker> for ValidationWorker {
    ...
    fn handle(&mut self, _msg: ShutdownWorker, ctx: &mut Self::Context) -> Self::Result {
        tracing::info!("ValidationWorker {} shutting down", self.id);
        ctx.stop();
    }
}
...
mod tests {
    use super::*;
    use crate::actors::network::tests::helpers::{SyncTestHarness, create_test_block};
